...
int main(int argc, char const *argv[]) {
	...
	while (loop) {
		...
		switch (opcode) {
			...
			case CONS:
			{
				int hd = pop(stack, top);
				int tl = pop(stack, top);
				int32_t heap_addr = new(heap, hd, tl);
				push(stack, top, heap_addr);
			}
			case HD:
			{
				int heap_addr = pop(stack, top);
				int hd = get_heap(heap_addr)->hd;
				push(stack, top, hd);
			}
			case TL:
			{
				int heap_addr = pop(stack, top);
				int tl = get_heap(heap_addr)->tl;
				push(stack, top, tl);
			}
			default:
				#ifdef __DEBUG__
